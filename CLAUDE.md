...
## 4. v1.1.0 实战调整（2026-05-12）

- **核心场景 = 持仓防守**：用户已持有股票仓位，App 的目的是盯盘防守而非寻找新买点。AI 唯一任务是保护利润、提示风险、捕捉日内做 T 买卖点。
- **AI 四选一输出**：【坚定格局】/【逢高减仓·倒T】/【逢低吸筹·正T】/【危险清仓】。绝不做买入推荐。
- **北向资金已砍**：A 股盘中不再披露实时北向资金。`NorthboundFetcher.kt` 已删除。
- **龙虎榜 = 静态标签 + SharedPreferences 持久化**：盘后数据，每日仅抓一次并持久化到 SharedPreferences（附带 cacheDate），每次访问检查日期过期自动刷新。
- **短线不看 PE/PB，看流通市值**：腾讯 API arr[45] 提取流通市值。规则：<50 亿游资可操控，>200 亿需板块级别资金共振。
- **四象限资金防伪**：①净流入+涨速>0.5%=主力真拉升 ②净流入+涨速<0=主力派发 ③净流出+涨速>0.5%=散户推升/无量空涨 ④净流出+涨速<-0.5%=主力真砸盘。Service 层 `buildContext()` 自动判定象限注入 Prompt。
- **Alpha 差值**：ConceptBlockFetcher.alphaDiff() 计算 `个股X% - 板块Y% = Z%`，差值>0 给予容忍度，差值<0 判定跟风杂毛。
- **数据源调度**：资金流向每 30 秒、概念板块每 5 分钟、龙虎榜按需（日期过期刷新）、全市场情绪每 5 分钟。无北向。
