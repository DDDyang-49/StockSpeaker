...
def load_config():
    ...
    return { "stock_code": "600519", "speak_interval": 15, "large_order_threshold": 500, "speak_price": True, "speak_pct": True, "speak_current_hand": True, "speak_amount": True, "speak_vol_ratio": True, "speak_speed": False, "speak_large_orders": True }
...
def parse_qq_data(raw_data, threshold):
...
def main(page: ft.Page):
    try:
        ...
        def update_config():
            ...
            app_config["large_order_threshold"] = int(large_input.value or 500)
        ...
        def update_loop():
            ...
            threshold = int(large_input.value or 500)
            while is_running[0]:
                raw = query_tencent_stocks(code)
                if raw:
                    item = parse_qq_data(raw, threshold)
                    if item:
                        if item['current_hand'] >= threshold:
                            action = "主动买入" if item['speed'] > 0 else "抛出" if item['speed'] < 0 else "成交"
                            t_str = time.strftime('%H点%M分')
                            interval_large_events.append((t_str, action, item['current_hand']))
        ...
        controls_group = ft.Column([
            ...
            ft.Row([large_input]),
            ft.Text("请勾选播报内容:"),
            ft.Row([cb_price, cb_pct, cb_speed, cb_amount, cb_vol, cb_hand, cb_large], wrap=True)
        ])
        ...
        page.add(
            ft.SafeArea(
                ft.Column([
                    ft.Row([ft.Text("摸鱼听盘", size=24, weight="bold"), btn_tips], alignment="spaceBetween"),
                    controls_group, 
        ...
        error_msg = f"发生致命错误: {str(e)}\n\n详情追踪:\n{traceback.format_exc()}"
        page.add(ft.Text("启动出错了！", size=20, color="red"))
